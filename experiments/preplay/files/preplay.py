...
EXPCONFIG = {
        ...
        "dataversion": 1,
        "dataid": "MONROE.EXP.PREPLAY.BROWSING",
        "nodeid": "fake.nodeid",
...
def run_exp(meta_info, expconfig, ifname,url,protocol):
    ...
    har_stats={}
    print ifname, url,protocol
    cmd=["./pReplay",ifname,"193.10.227.23",url,protocol]
...
def create_exp_process(meta_info, expconfig,ifname,url,protocol):
    process = Process(target=run_exp, args=(meta_info, expconfig,ifname,url,protocol))
    process.daemon = True
    return process
...
#
        ...
        exp_process = exp_process = create_exp_process(meta_info, EXPCONFIG, ifname, sys.argv[1],sys.argv[2])
        exp_process.start()
