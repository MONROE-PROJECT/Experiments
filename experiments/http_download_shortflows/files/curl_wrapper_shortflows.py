...
r"""
Simple curl wrapper download a many small segments using curl.
...
def run_exp(curl_cmd):
    """Run the experiment return the output."""
...
def create_arg_parser():
    ...
    parser.add_argument('--segmentsize',
                        type=int,
                        required=True,
                        help="Size of each segment (B).")
    parser.add_argument('--segments',
                        nargs='+',
                        type=int,
                        default=[1],
                        help="Segments to download (default 1)")
...
if __name__ == '__main__':
    ...
    data_id = 'MONROE.EXP.HTTP-DOWNLOAD-SHORTFLOWS'
    data_version = 1
    interval = args.interval
    export_interval = interval*5  # 4 experiment runs in each json file
    segments = args.segments
    segment_size = args.segmentsize - 1  # account for the first byte
