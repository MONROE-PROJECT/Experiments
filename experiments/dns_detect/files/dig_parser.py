...
# # reading from file -- testing
#     lines = [line.rstrip() for line in dig_output] # All lines including the blank ones
#     digResult = [line for line in lines if line] # Non-blank lines

# reading from dig output -- container
    lines = dig_output.strip().split("\n")
    #print str(lines)
    digResult = [line for line in lines if line]
    #print "DIG output: " + "\n".join(digResult)
    ...
    queryTime = lines[-4].split(" ")[3].strip()
    server = lines[-3].split(" ")[2].strip()
    time = lines[-2][lines[-2].find(": ") + 2:].strip()
    messageSize = lines[-1].split(" ")[-1].strip()
